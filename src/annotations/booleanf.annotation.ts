import type { AnySchema, BooleanSchema, SchemaInternals } from 'joi'
import Joi from 'joi'
...
import * as b from '~/op/boolean.op'

type SchemaNoInternal = Diff<BooleanSchema, SchemaInternals>
...
export function BooleanF(...args: Partial<Options>[]): FChain {
...
function annotate(target: any, propertyKey: string, options: Partial<Options> = {}) {
  let item: AnySchema = Object.entries(options)
    .reduce((acc, [opt, arg]) => (acc as any)[opt](...arg), Joi.boolean())
